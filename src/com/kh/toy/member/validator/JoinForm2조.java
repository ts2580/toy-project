...
public class JoinForm2조 {
	...
	public JoinForm2조(HttpServletRequest request) {
	...
	public boolean test() {
		...
		//비밀번호가 영수특문 8-12자리
		vaild = Pattern.matches("^(?=.*[A-Za-z])(?=.*\\d)(?=.*[$@$!%*#?&])[A-Za-z\\d$@$!%*#?&](.{8,13}$)", password);
		if (!vaild) {
		...
		// 이름, 한글만. 1-5자
		vaild = Pattern.matches("(?=.*[ㄱ-힣])(.{1,6})", name);
		if (!vaild) {
		...
		// 생년월일
		// 연/월/일
		// year까지는 저희가 validator 지정-숫자만,1900-2020까지? 월, 일은 옵션으로 받기
		vaild =  Pattern.matches("^(19[0-9][0-9]|20[0-2][0-1])$", year);
		if (!vaild) {
			failedAttribute.put("year", year);
			res = false;
		}
